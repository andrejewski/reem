...
Reem.prototype.diffBuild = function(filepath, next) {
	...
	this.read({sourcePath: path.dirname(filepath)}, function(error, branch) {
		if(error) return next(error);
		(function findAndReplace(list) {
			if(list.sourcePath === branch.sourcePath) {
				list = extend(list, branch);
...
Reem.prototype.write = function(tree, next) {
	...
	function writeList(list, done) {
		...
		function writePost(post, done) {
			...
			renderFile(post, function(error, html) {
				if(error) return done(error);
				if(typeof post.outputPath !== 'string') {
					console.log('#rf', post);
				}
		...
		function writeFile(file, done) {
			...
			if(file.renderFile) return renderFile(file, function(error, html) {
				if(error) return done(error);
	...
	function writePage(page, next) {
		reem.Page.run(page, function(error, page) {
			...
			renderFile(page, function(error, html) {
				if(error) return next(error);
	...
	function renderFile(item, next) {
		item.view = item.view || item.filetype+(reem.view.extension || "."+reem.view.engine);
		var template = path.join(reem.fs.layout.filepath, item.view),
			render = consolidate[reem.view.engine],
			locals = extend({}, reem.view, {g: reem.data});
