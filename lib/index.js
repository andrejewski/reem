...
Reem.prototype.build = function(tree, next) {
	...
	if(tree.sourcePath === this.fs.source.sourcePath) process.unshift(this.read);
...
Reem.prototype.diffBuild = function(filepath, next) {
	var reem = this,
		tree = this._tree;
	if(tree && filepath.indexOf(tree.sourcePath) === -1) return this.build(tree, next);
	if(!reem.fs.readAccept(path.basename(filepath))) {next(null); return this;}
	if(!tree) return this.build(next);
	...
	(function readList(list, next) {
		...
		function readdir(dir, done) {
			fs.readdir(dir, function(error, files) {
				...
				done(error, files.filter(reem.fs.readAccept));
			});
		}
			...
			function mergeFile(files) {
				...
				return function(file, done) {
					...
					mergeData(file, files.filter(function(f) {
						var support = file.readType && file.basename === f.basename && file !== f;
